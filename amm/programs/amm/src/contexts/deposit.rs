...
pub struct Deposit<'info> {
    ...
    /// TODO: remove authorit from PDA seeds
    #[account(
        has_one = mint_x,
        has_one = mint_y,
        seeds = [
...
impl<'info> Deposit<'info> {
    pub fn deposit(&self, amount: u64, max_x: u64, max_y: u64, expiration: i64) -> Result<()> {
        ...
        let (x, y) =
            match self.mint_lp.suppy == 0 && self.vault.amount == 0 && self.vault_y.amount == 0 {
                true => (max_x, max_y),
                false => {
                    let amounts = ConstantProduct::xy_deposit_amounts_from_1(
                        self.vault_x.amount,
                        self.vault_y.amount,
                        self.mint_lp.supply,
                        amount,
